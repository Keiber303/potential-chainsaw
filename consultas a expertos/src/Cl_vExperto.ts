...
export default class Cl_vExperto extends Cl_vGeneral {
    ...
    constructor() {
        ...
        this.lblOpcion = this.crearHTMLLabelElement("lblOpcion", {
         ...
         this.inId = this.crearHTMLInputElement("id",  {
            oninput: () => {
                this.inId.value = (this.experto.id as any) = this.inId.value
                .toUpperCase()
                .trim();
                this.refresh();
            },
            refresh: () =>
            (this.inId.style.borderColor = this.experto.idOk ? "" :"red"),
        });
    ...
    aceptar() {
        if (this.opcion === opcionFicha.add)
            this.controlador!.addExperto({
                dtExperto: this.experto.toJSON() as any,
    ...
    show(
        ...
        if (opcion) {
            this.opcion = opcion;
            (this.experto.id as any) = this.inId.value =(this.experto.id as any);
